...
struct page_struct *_new_page(int type, void *virt_address, bool writable,
...
bool _operation_page(int operation, void *address, void * kpage, bool pinned)
{
	...
	if (operation == OP_LOAD)
	{
		...
		if (page->physical_address == NULL)
			page->physical_address = _get_frame(NULL, NULL, PAL_USER);
		...
		if (page->type == TYPE_FILE)
		{
			...
			{
				_free_frame(page->physical_address, page->pagedir);
				success = false;
			}
			else
			{
		...
		if (!success)
		{
			_pin(false, page->physical_address, true);
			return false;
		}
...
{
	struct page_struct *page = NULL;
	page = _new_page(TYPE_ZERO, address, true, NULL,
	NULL, NULL, NULL, NULL);
	if (page != NULL)
	{
		bool success = _operation_page(OP_LOAD, page, page->physical_address,
				pin);
		if (success)
			return page;
		else
