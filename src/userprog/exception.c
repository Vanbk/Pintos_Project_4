...
static void page_fault(struct intr_frame *f)
{
	...
	// is vitual addres that cause the page fault
	...
	//obtain fault page
	// get the page number PTE = 0xfffff000 (20 first bits is page number)
	fault_page = PTE_ADDR & (uint32_t) fault_addr;

	struct page_struct *page;

	page = _find_page(fault_page);

	if (page != NULL)
	{
		if (write && !page->writable)
			system_call_exit(-1);
		bool success = _operation_page(OP_LOAD, page, page->physical_address, false);
		if (success)
			return;
		else
			system_call_exit(-1);
	...
	if (stack_status)
	{
		struct page_struct *temp = _stack_grow(fault_page, false);
		if (temp != NULL)
			return;
		else
			system_call_exit(-1);
