...
#else
					_free_frame(pte_get_page(*pte), pd);
#endif
				}
#ifndef VM
				palloc_free_page (pt);
					...
					_operation_page(OP_FREE, (struct page_struct *) *pte,
					NULL,
					NULL);
			_free_frame(pt, pd);
#endif
...
lookup_page(uint32_t *pd, const void *vaddr, bool create)
{
	...
	// pde_get_pt Returns a pointer to the page table that PDE point to

	pt = pde_get_pt(*pde);
	//pt_no Obtains page table index from a virtual address
...
void *pagedir_op_page(uint32_t *pd, void *uaddr, void *vm_page)
{
	...
	if (vm_page == NULL)
	{
		//Returns the address of PTE for virtual
 		//address UADDR in page directory PD.
		entry = lookup_page(pd, uaddr, false);
		if (entry != NULL)
		{
			//PTE_P 0x1      1=present, 0=not present.
			if ((*entry & PTE_P) != 0)
			{
				// Returns a pointer to the page that page table 
				// entry PTE points to.
				void *kpage = pte_get_page(*entry) + pg_ofs(uaddr);
				void *addr = _get_frame(kpage, pd, PAL_USER);
				return addr;
			}
			else
			{
	...
	else
	{
		...
		entry = lookup_page(pd, uaddr, true);

		if (entry != NULL)
		{
			ASSERT((*entry & PTE_P) == 0);
			*entry = (uint32_t) vm_page;
