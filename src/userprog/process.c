		...
		uint32_t read_bytes, uint32_t zero_bytes, bool writable)
{
	...
	while (read_bytes > 0 || zero_bytes > 0)
	{
		...
		p = NULL;
		p = _new_page(TYPE_FILE, upage, writable, file, load_offset,
...
static bool setup_stack(void **esp)
{
	...
	p = NULL;
	p = _new_page(TYPE_ZERO, ((uint8_t *) PHYS_BASE) - PGSIZE, true, NULL,
			NULL, NULL, NULL, NULL);
	if (p != NULL)
	{
		*esp = PHYS_BASE;
		_operation_page(OP_LOAD, p, p->physical_address, false);
		success = true;
	}
#endif
	return success;
