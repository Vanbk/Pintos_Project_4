...
int system_call_read(int fd, void *buffer, unsigned size)
{
	...
	default:
				...
				remaining = remaining - bytes_read)
		{
			...
			struct page_struct *page = _find_page(address);

			if (page == NULL)
			if (stack_status)
			page = _stack_grow(temp_buffer - offset, true);
			else
			system_call_exit(-1);

			//load the page and pin it
			if (!page->loaded)
			_operation_page(OP_LOAD, page, page->physical_address, true);
...
void system_call_munmap(mapid_t mapid)
{
	...
	if (mf != NULL)
	{
		...
		{
			page = _find_page(cur_addr);
			if (page != NULL)
			{
				if (page->loaded)
				{
					_pin(true, page, false);
				...
				_operation_page(OP_FREE, page, NULL, NULL);
				cur_addr += PGSIZE;
				page = NULL;
			}
		}
...
mapid_t system_call_mmap(int fd, void *address)
{
	...
	while (size > 0)
	{
		...
		struct page_struct * temp = _find_page(tmp_addr);
		if (temp == NULL)
		{

			temp = _new_page(TYPE_FILE, tmp_addr, true, f, offset,
